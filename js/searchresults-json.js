	 ...
	 if ($('#courselistdynamic').length) {
		 ...
		 if (selectedValue == 'All') {
			...
			if ($('.sys_level-UG').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no undergraduate courses. </p><p>Please try searching for another term or choose a different category.</p></li>');
			}
		 }
			...
			if ($('.sys_UG-Clearing').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no courses that currently have places.</p><p>Please try searching for another term.</p></li>');
			}
		 }
			...
			if ($('.sys_level-App').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no apprenticeships.</p><p>Please try searching for another term or choose a different category.</p></li>');
			}
		 }
			...
			if ($('.sys_level-Top').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no top-up degrees.</p><p>Please try searching for another term or choose a different category.</p></li>');
			}
		}
			...
			if ($('.sys_level-FY').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no courses that include a foundation year.</p><p>Please try searching for another term or choose a different category.</p></li>');
			}
		 }
		 else {
			$('#courselistdynamic > li').hide();
			if ($('.sys_level-PG').length) {
				...
				$('#courselistdynamic').append('<li class="sys_level-NONE"><p>Your search returned no postgraduate courses. </p><p>Please try searching for another term or choose a different category.</p></li>');
			}
		 } 
...
function searchForTerm(searchTerm, courseArray){
    ...
    for (var i=0; i < courseArray.length; i++) {
		...
		var keywords = courseArray[i].keywords;
		...
		for (var j=0; j < searchTermsArray.length; j++) {
			...
			// then search course keywords
            if (regSearch.test(keywords)) {
                courseArray[i]['weight'] = courseArray[i]['weight'] + 10;
                pushCourse = true
            }
...
function buildCourseResult(course) {
	...
	if (course.title.indexOf('top-up') > -1) {
  	...
  	var foundationYearClass = (course.title.indexOf('foundation year') > -1) || (course.title.indexOf('preparatory year') > -1) ? ' sys_level-FY' : ''
	...
	if (course.level == 'UG') {
		resultOutput = '<li id="courseNode-' + course.nodeId + '" class="sys_level-UG ' + topUpClass + foundationYearClass + '"><a href="'+ course.link +'"><article><h2>'+ course.title +'</h2>' + straplineOutput + '<div class="sys_coursedefs"><h3>Location</h3><ul class="sys_definitions">' + locationsOutput + '</ul></div><div class="sys_coursedefs  sys_resultduration"><h3 class="sys_durationhead">Mode of study</h3><ul class="sys_definitions sys_resultduration">' + modesOutput + '</ul></div><div class="sys_coursedefs sys_resultucas"><h3>UCAS code</h3><ul class="sys_definitions sys_resultucas"><li>' + course.ucas + '</li></ul></div></article></a></li>';
	}
	...
	if (matchingCourses.length > 0 ) {
		...
		if (levelToShow != 'All') {
			if (levelToShow == 'UG') {
				...
				if (!$('.sys_level-FY').length) {
					$('#courselistdynamic').append('<li class="sys_level-NONE"><pYour search returned no courses that include a foundation year.</p><p>Please try searching for another term or choose a different category.</p></li>');
				}
			}
			else {
				$('.sys_level-PG, .sys_level-App').hide();
